...
def _mask_pruned_skeleton(thr, prefilter_radius, prune_length):
  ...
  thr_fil = thr #skimage.filters.gaussian(thr, sigma=prefilter_radius) > 0.5
  # make skeleton
  skeleton = skimage.morphology.skeletonize(thr_fil)
  _, distances = skimage.morphology.medial_axis(thr_fil, return_distance=True)
  ...
  skeleton  = skimage.morphology.medial_axis(skeleton, return_distance=False)
  skeleton = skeleton.astype(numpy.uint8)
...
def cell_kn_analysis(cell_image, min_area=17, kn_threshold_area=250):
  ...
  pth_props_table = skimage.measure.regionprops_table(dna_lab, cell_image.phase_mask,
                                                      properties=("intensity_max", "area_convex"))

  dna_props_table = skimage.measure.regionprops_table(dna_lab, dna,
                                                      properties=("centroid", "minor_axis_length", "major_axis_length",
                                                                  "orientation",
  ...
  for i in range(0, dna_lab.max()):
    ...
    if pth_props_table["intensity_max"][i] == 255 and pth_props_table["area_convex"][i] > min_area:
      # get stats
      dna_objects.append({
        "centroid": {
        ...
        "orientation": dna_props_table["orientation"][i],
        "intensity_mean": dna_props_table["intensity_mean"][i],
        "intensity_max": dna_props_table["intensity_max"][i],
      })
...
def cell_midline_analysis(cell_image, prefilter_radius=2, min_length_pruning=15):
  ...
  """
  pth_skeleton, _ = _mask_pruned_skeleton(cell_image.phase_mask, prefilter_radius, min_length_pruning) # MAGIC NUMBERS: Radius for prefiltering, length for pruning branches
