...
class CellLine:
  ...
  """
    ...
    def __str__(self):
        return " ".join(
            [str(x) for x in ["gene_id =", self.gene_id, "terminus =", self.terminus, "life_stage =", self.life_stage]])
...
class CellImage():
  ...
  """
    ...
    def __repr__(self):
        string = "\n".join([str(x) for x in
                            ["phase", self.phase, "mng", self.mng, "dna", self.dna, "phase_mask", self.phase_mask,
                             "dna_mask", self.dna_mask, "phase_mask_othercells", self.phase_mask_othercells]])
        if self.cell_line is not None:
            string += "\n" + str(self.cell_line)
        string += "\n" + " ".join([str(x) for x in
                                   ["field_index =", self.field_index, "cell_index =", self.cell_index, "rotated =",
                                    self.rotated]])
        return string
...
class TrypTag:
    ...
    """
        ...
        while response is None:
            try:
                ...
                time.sleep(
                    60)  # MAGIC NUMBER: testing shows the rate limiter resets after 50s, so 60s for a bit of space
    ...
    """
        ...
        lines = [l for l in (line.strip() for line in lines) if l]
    ...
    """
        ...
        return [
            ...
            ((gene_id, terminus) for gene_id, gene_entry in self.gene_list[life_stage].items() for terminus in
             self.termini if terminus in gene_entry)
        ]
    ...
    """
        ...
        return [
            ...
            ((gene_id, terminus) for gene_id, gene_entry in self.gene_list[life_stage].items() for terminus in
             self.termini if "wild-type" in gene_id and terminus in gene_entry)
        ]
    ...
    """
      ...
      """
        ...
        return _parse_localisation_ontology(
            json.loads(self._fetch_zenodo_record_file(zenodo_json, "localisation_ontology.json")))

    def _localisation_match(self, cell_line, query_term: str, match_subterms: bool = True,
                            exclude_modifiers: list = ["weak", "<10%"], required_modifiers: list = None) -> bool:
        """
    ...
    def localisation_search(self, query_term: str, life_stage: str = None, match_subterms: bool = True,
                            exclude_modifiers: list = ["weak", "<10%"], required_modifiers: list = None) -> list:
        """
    ...
    """
        ...
        for cell_line in self.worklist_all(life_stage):
            if self._localisation_match(cell_line, query_term, match_subterms=match_subterms,
                                        exclude_modifiers=exclude_modifiers, required_modifiers=required_modifiers):
    ...
    """
        ...
        if free < space_required:
            if self.print_status: print("! Insufficient free disk space for full data cache: " + str(
                round(free / float(2 << 40), 4)) + " / " + str(
                round(space_required / float(2 << 40), 4)) + " TiB available !")
    ...
    :param cell line: `CellLine` object containing `life_stage`, `gene_id` and `terminus`.
    """
        base_path = os.path.join(self.data_cache_path,
                                 self.gene_list[cell_line.life_stage][cell_line.gene_id][cell_line.terminus]["plate"])
        if "cells" not in self.gene_list[cell_line.life_stage][cell_line.gene_id][cell_line.terminus] and os.path.isdir(
                base_path):
            cells = []
            for i in range(len(glob.glob(os.path.join(base_path,
                                                      cell_line.gene_id + "_4_" + cell_line.terminus.upper() + "_*_roisCells.txt")))):
                with open(os.path.join(base_path, cell_line.gene_id + "_4_" + cell_line.terminus.upper() + "_" + str(
                        i + 1) + "_roisCells.txt")) as cells_file:
            ...
            self.gene_list[cell_line.life_stage][cell_line.gene_id][cell_line.terminus]["cells_per_field"] = [len(x) for
                                                                                                              x in
                                                                                                              cells]
            self.gene_list[cell_line.life_stage][cell_line.gene_id][cell_line.terminus]["cells"] = cells.copy()
            if self.print_status: print("  Counted",
                                        self.gene_list[cell_line.life_stage][cell_line.gene_id][cell_line.terminus][
                                            "fields_count"], "image data files with", sum(
                    self.gene_list[cell_line.life_stage][cell_line.gene_id][cell_line.terminus]["cells_per_field"]),
                                        "cells for", cell_line.life_stage, cell_line.gene_id, cell_line.terminus)
      ...
      """
        ...
        try:
            if not os.path.isfile(zip_path) and not os.path.isfile(os.path.join(dir_path, "_" + plate + ".zip.md5")):
                ...
                print(
                    "Fetching data for " + cell_line.life_stage + " form, gene ID " + cell_line.gene_id + " tagged at " + cell_line.terminus + " terminus")
                ...
                if "record_id" not in self.zenodo_index[plate]:
                    ...
                    # fetch Zenodo record JSON, to get latest version doi
                    zenodo_json = self._fetch_zenodo_record_json(
                        self.gene_list[cell_line.life_stage][cell_line.gene_id][cell_line.terminus]["zenodo_id"])
                ...
                while zip_md5 != self.zenodo_index[plate]["record_md5"]:
                    if self.print_status:
                        ...
                        with _tqdmDownload(unit='B', unit_scale=True, unit_divisor=1024, miniters=1,
                                           desc=plate) as progress:
                            urllib.request.urlretrieve(self.zenodo_index[plate]["record_url"], zip_path_temp,
                                                       progress.urllib_callback)
                    else:
                        urllib.request.urlretrieve(self.zenodo_index[plate]["record_url"], zip_path_temp)
                    ...
                    if zip_md5 != self.zenodo_index[plate]["record_md5"]:
                        if self.print_status: print(
                            "! MD5 of downloaded zip is incorrect (" + zip_md5 + "), retrying download !")
                if self.print_status: print("  Download complete")
                shutil.move(zip_path_temp, zip_path)
                with open(zip_path + ".md5", "w") as file:
                    file.write(zip_md5)
        finally:
            lock.release()
                ...
                if not os.path.isfile(os.path.join(dir_path, "_" + plate + ".zip.md5")):
                    ...
                    try:
                        with ZipFile(zip_path) as archive:
                            ...
                            missing = []
                            # do decompression
                            for file in tqdm(archive.namelist(), unit=' files', miniters=1, desc=plate,
                                             disable=not self.print_status, smoothing=0):
                                ...
                                if file.endswith(suffix) and not (
                                        os.path.split(file)[-1].startswith("control") or os.path.split(file)[
                                    -1].startswith("ontrol") or os.path.split(file)[-1].startswith("Control")):
                                    ...
                                    if file_roi in archive.namelist() and file_tif in archive.namelist() and file_thr in archive.namelist():
                                        ...
                                        shutil.move(os.path.join(self.data_cache_path, file_roi),
                                                    os.path.join(target_path, os.path.split(file_roi)[-1]))
                                        shutil.move(os.path.join(self.data_cache_path, file_tif),
                                                    os.path.join(target_path, os.path.split(file_tif)[-1]))
                                        shutil.move(os.path.join(self.data_cache_path, file_thr),
                                                    os.path.join(target_path, os.path.split(file_thr)[-1]))
                                    else:
    ...
    """
        ...
        fieldcell_list = []
        for field_index, cell_list in enumerate(
                self.gene_list[cell_line.life_stage][cell_line.gene_id][cell_line.terminus]["cells"]):
                        ...
                        if self.gene_list[life_stage][cell_line.gene_id][cell_line.terminus]["plate"] == plate:
                            if "cells" in self.gene_list[life_stage][cell_line.gene_id][cell_line.terminus]:
                                ...
                                if len(self.gene_list[life_stage][cell_line.gene_id][cell_line.terminus]["cells"]) == 0:
                                    print("  No images found, but images expected, for", life_stage, gene_id, terminus,
                                          "in", plate)
    ...
    """
        ...
        field_base_path = os.path.join(self.data_cache_path,
                                       self.gene_list[cell_line.life_stage][cell_line.gene_id][cell_line.terminus][
                                           "plate"], cell_line.gene_id + "_4_" + cell_line.terminus.upper() + "_" + str(
                field_index + 1))
        if field_base_path != self._field_base_path_sk:
            ...
            field_image = numpy.moveaxis(field_image, [0, 1, 2],
                                         [1, 2, 0])  # MAGIC NUMBER: correct loading in a weird order
    ...
    def _open_cell(self, cell_line, field_index: int, crop_centre: tuple, fill_centre: tuple, custom_field_image=None,
                   rotate: bool = False, angle: float = 0, width: int = 323) -> list:
        """
    ...
    """
        ...
        if width < 0:
            ...
            for channel in channels:
                ...
                offs = 0
                if ymin - padding < 0 or xmin - padding < 0 or ymax + padding > channel.shape[0] or xmax + padding > \
                        channel.shape[1]:
                    channel = numpy.pad(channel, ((padding, padding), (padding, padding)), mode="median")
                    offs = padding
                # crop
                cell_channels.append(
                    channel[ymin - padding + offs:ymax + padding + offs, xmin - padding + offs:xmax + padding + offs])
        elif width > 0:
            # fixed width mode
            if rotate:
            ...
            for channel in channels:
                ...
                if crop_centre[0] - half_width_inter < 0 or crop_centre[1] - half_width_inter < 0 or crop_centre[
                    0] + half_width_inter > channel.shape[1] or crop_centre[1] + half_width_inter > channel.shape[0]:
                    channel = numpy.pad(channel,
                                        ((half_width_inter, half_width_inter), (half_width_inter, half_width_inter)),
                                        mode="median")
                    offs = half_width_inter
                # square crop
                channel = _skimage_crop(channel, crop_centre[0] + offs - half_width_inter,
                                        crop_centre[1] + offs - half_width_inter, width_inter, width_inter)
                if rotate:
                    ...
                    channel = _skimage_crop(channel, half_width_inter - width / 2, half_width_inter - height / 2, width,
                                            height)
                cell_channels.append(channel)
        else:
            raise ValueError(
                "`width` must be a nonzero integer, positive for fixed image width in pixels, negative for padding around phase_mask in pixels.")
    ...
    def open_cell(self, cell_line, field_index: int = 0, cell_index: int = 0, width: int = 323,
                  rotate: bool = False) -> list:
        """
    ...
    """
        if cell_line.life_stage is None:
            cell_line.life_stage = self.life_stages[0]
        self.fetch_data(cell_line)
        cell_data = self.gene_list[cell_line.life_stage][cell_line.gene_id][cell_line.terminus]["cells"][field_index][
            cell_index]
        ...
        if width < 0:
            rotate = False
        phase, mng, dna, phase_mask, dna_mask, phase_mask_othercells = self._open_cell(cell_line, field_index,
                                                                                       crop_centre, fill_centre,
                                                                                       angle=angle, rotate=rotate,
                                                                                       width=width)
        return CellImage(
            phase=phase,
            mng=mng,
    ...
    def open_cell_custom(self, cell_line, field_index: int = 0, cell_index: int = None, custom_field_image=None,
                         fill_centre: tuple = None, crop_centre: tuple = None, rotate: bool = False,
                         angle: float = None, width: int = 323) -> list:
        """
    ...
    """
        ...
        if cell_index is not None:
            cell_data = \
            self.gene_list[cell_line.life_stage][cell_line.gene_id][cell_line.terminus]["cells"][field_index][
                cell_index]
            ...
            if fill_centre is None:
                raise ValueError(
                    "If `cell_index` is not set or `None` then `fill_centre` must be set and not be `None`")
    ...
    """
        ...
        with Executor(workers) as executor:
            futures = [
                executor.submit(self._list_analysis_worker, cell_line=cell_line, analysis_function=analysis_function,
                                threading_mode=multiprocess_mode == "thread") for cell_line in dedup_work_list]
            results = [future.result() for future in
                       tqdm(concurrent.futures.as_completed(futures), total=len(futures), smoothing=0)]
        return results
...
class BSFTag(TrypTag):
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(self, *args, **kwargs, master_zenodo_id=7258722, data_cache_path="./_bsftag_cache",
                         data_cache_plates=4, life_stages=["bloodstream"])
